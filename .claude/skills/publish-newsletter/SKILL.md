...
| **7** | **Self-check** against the twelve-point publication checklist, the Moat Check, and `check-voice.mjs`. A Moat Check that fails any of its three gates blocks Step 8 the same way a `NO PUBLICAR TODAVÍA` at Step 0.5 does — this funnel has no human review to catch it later. | `references/voice-and-style.md` §12, `references/moat-check.md` |
