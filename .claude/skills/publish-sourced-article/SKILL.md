...
| **2c** | **Editorial gate — does this item earn space in Playbook at all?** PUBLICAR / NO PUBLICAR TODAVÍA / RADAR. A different question from 2b's sensitivity. Anything other than PUBLICAR means don't draft it — say so instead when presenting the batch to the human, rather than spending a review round on it. | `references/editorial-gate.md` |
...
into `.claude/playbook-editorial/`, shared with `publish-newsletter` and
